...
int main(int argc, char **argv) 
{
	...
	char C;				/* carattere passato come ultimo parametro */
	int fd; 			/* file descriptor per apertura file */
	long int pos; 			/* valore per comunicare la posizione all'utente */
	...
	printf("Sono il padre con pid %d e creo %d figli che dovranno cercare il carattere %c\n", getpid(), N, C);
...
/* Ciclo di generazione dei figli */
	for (i=0; i < N; i++)
	{
		...
		if (pid == 0) 
		{
			...
			{
				if (ch == C)
				{ /* il figlio ha trovato il carattere cercato e quindi bisogna comunicare al padre la posizione: si puÃ² far, ad esempio, usando la funzione lseek; consideriamo le posizioni dei caratteri a partire dalla posizione 1 (dato che abbiamo gia' letto il carattere il valore restituito dalla lseek sara' gia' quello giusto) */
			...
			/* il figlio Pi deve ritornare al padre il valore corrispondente al numero di sostituzioni effettuate (che si puo' supporre strettamente minore di 255 */
			exit(ritorno);
  		}	
	}
